...
def center_coordinates(window: pygame.Surface, other: pygame.Surface | Rectangle | Tuple[int, int] | None = None) -> Tuple[int, int]:
  ...
  if isinstance(other, Rectangle):
    ...
    return (window.get_width() // 2 - other.width // 2, window.get_height() // 2 - other.height // 2)
  
  if isinstance(other, tuple):

    return ((window.get_width() - other[0]) // 2, (window.get_height() - other[1]) // 2)
  
  return ((window.get_width() - other.get_width()) // 2, (window.get_height() - other.get_height()) // 2)